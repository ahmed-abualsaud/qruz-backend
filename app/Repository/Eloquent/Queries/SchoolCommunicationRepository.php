    ...
    public function schoolTripChatMessages(array $args)
    {
        try {
            $messages = $this->model->with('sender:id,name')
                ...
                ->selectRaw('school_trip_chat.*, DATE_FORMAT(created_at, "%l:%i %p") as time');
    ...
    public function schoolTripPrivateChatUsers(array $args)
    {
        return $this->model->select('users.id', 'users.name', 'users.avatar')
            ...
            ->join('users', function ($join) {
                $join->on('users.id', '=', 'school_trip_chat.sender_id')
                    ->where('sender_type', 'App\User');
                $join->orOn('users.id', '=', 'school_trip_chat.recipient_id')
